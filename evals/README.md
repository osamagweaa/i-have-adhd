...
Project memory arrives by a second door that runner flags do not close: agents read `CLAUDE.md`, `GEMINI.md`, and `AGENTS.md` from their working directory, and this repo's root holds two of them. Every call therefore runs from an empty scratch directory rather than the repo root, so the repo's own ruleset cannot leak into a condition. Nothing depends on the old working directory: the runner receives only its command and the prompt, and the condition skill is read by `run_evals.py` itself.
